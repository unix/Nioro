...
function addon:OnInitialize()
  local setTexture = function(frame)
    ...
    if NIORO_DB.SETTINGS.USE_FLAT_TEXTURE then
      frame.healthBar:GetStatusBarTexture():SetColorTexture(0.75, 0.75, 0.75)
    end
  end
  ...
  hooksecurefunc('CompactUnitFrame_SetUnit', function(f, unit)
    ...
    if not Utils:isRaidFrame(f) then
      return
    end
  ...
  hooksecurefunc('CompactUnitFrame_UpdateName', function(f)
    ...
    if NIORO_DB.SETTINGS.FONT_NAME_SCALE then
      ...
      --local baseFontSize = fontSize or infos.NATIVE_FONT_SIZE
      --local nextFontSize = baseFontSize * NIORO_DB.SETTINGS.FONT_NAME_SCALE
      f.name:SetScale(NIORO_DB.SETTINGS.FONT_NAME_SCALE)
      --f.name:SetFont(fontName, nextFontSize, fontFlags)
    end
  end)
  ...
  hooksecurefunc('CompactUnitFrame_UpdateStatusText', function(f)
    ...
    if not f.statusText or not f.statusText:IsShown() then
      return
    end
    if NIORO_DB.SETTINGS.FONT_STATUS_SCALE then
      --local fontName, fontSize, fontFlags = f.statusText:GetFont()
      --local baseFontSize = infos.NATIVE_FONT_SIZE
      --local nextFontSize = baseFontSize * NIORO_DB.SETTINGS.FONT_STATUS_SCALE
      --f.statusText:SetFont(fontName, nextFontSize, fontFlags)
      f.statusText:SetScale(NIORO_DB.SETTINGS.FONT_STATUS_SCALE)
    end
  ...
  hooksecurefunc('CompactUnitFrame_UpdateRoleIcon', function(f)
    if not f or not f.roleIcon then
      return
    end
