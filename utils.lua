...
function Utils:isDefaultSetting(key, val)
  ...
  if NIORO_DB.DEFAULT_SETTINGS[key] == nil then
    return true
  end
...
function Utils:isRaidFrame(f)
  ...
  if f.unit and string.find(f.unit, 'nameplate') then
    return false
  end
  ...
  if not isRaidFrame and not isPartyFrame and not isRaidGroupFrame then
    return false
  end
  return true
end
